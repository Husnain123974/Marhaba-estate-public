...
const ContactForm = ({ isShowMessage = false, headingText = '' }: ContactFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      name,
      email,
      phone: `${countryCode} ${phone}`, // Full phone number with country code
      message,
    };
    ...
    <div className="bg-black text-white flex justify-center items-center p-8 mt-7">
      <div className="bg-[#1E1E1E] w-[100%]  h-[auto] min-h-[445px] p-8 rounded-[1.5rem] shadow-lg flex flex-col justify-center items-center">
        <div className="flex flex-col items-center justify-center mb-8">
          <h2 className="text-3xl font-bold text-center mb-2">{headingText || defaultTitle}</h2>
          <p className="text-gray-400 text-center">Fill form below and our agent will contact you shortly</p>
      </div>
